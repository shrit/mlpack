...
| `-DMLPACK_USE_ZLIB` | `#define MLPACK_USE_ZLIB` | Enable zlib-based gzip decompression for datasets.  When enabled, `Load()` will automatically decompress `.gz` files.  Requires linking with zlib (`-lz`). |
